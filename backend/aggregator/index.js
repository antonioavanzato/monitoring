...
function calibrate(payload) {
  ...
  //
  // Верхняя граница была 6 — и это отключало поправку там, где она нужнее
  // всего. У ALGA намерено 6,39: вызовы редкие (таймер раз в две минуты),
  // поэтому каждый виден в большем числе точек, чем при плотном потоке.
  // Отказ от поправки не осторожность, а худший исход: сумма остаётся
  // завышенной в шесть раз, и карточка врёт молча. 10 — потолок с запасом:
  // скользящая минута при шаге 15 секунд даёт максимум 5 точек на вызов,
  // удвоение возможно при двух сериях на одну функцию.
  if (!isFinite(k) || k < 1 || k > 10) return null;
  return k;
}
