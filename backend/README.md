...
Скрипт спросит, какой профиль `yc` отвечает за какой проект, и пароль для
дашборда — остальное сделает сам: сервисные аккаунты, права, ключи, Lockbox,
деплой обеих функций, API Gateway, запись адреса в `config.js`. В конце
покажет команду `git push`, после которой дашборд оживёт.

Запускать можно повторно — всё создаётся идемпотентно.

Нужны только `node` и `yc` с настроенными профилями (`yc init` по разу на
каждое облако).

---

## Ручной путь — те же шаги по отдельности

Если `setup.sh` где-то споткнулся, ниже то же самое вручную.
